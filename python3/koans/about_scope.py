...
class AboutScope(Koan):
    ...
    def test_incrementing_with_local_counter(self):
        global counter # this is global value 'counter' 0
        start = counter # this is local here not global... 0
        self.increment_using_local_counter(start) # no change, still 0
        self.assertEqual(False, counter == start + 1)

    def test_incrementing_with_global_counter(self):
        global counter # 0
        start = counter # 0
        self.increment_using_global_counter() # now counter = 1, start = 0
