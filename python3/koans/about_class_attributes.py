...
class AboutClassAttributes(Koan):
    ...
    def test_classmethods_are_not_independent_of_instance_methods(self):
        fido = self.Dog2()
        self.assertRegexpMatches(fido.growl(), 'classmethod growl')
        self.assertRegexpMatches(self.Dog2.growl(), 'classmethod growl')

    def test_staticmethods_are_unbound_functions_housed_in_a_class(self):
        self.assertRegexpMatches(self.Dog2.bark(), 'staticmethod bark')

    def test_staticmethods_also_overshadow_instance_methods(self):
        fido = self.Dog2()
        self.assertRegexpMatches(fido.bark(), 'staticmethod bark')
